...
public class CourtListenerClient : ICourtListenerClient
{
    ...
    public CourtListenerClient(HttpClient httpClient, IConfiguration configuration)
    {
        _httpClient = httpClient;
        _baseUrl = configuration["CourtListener:BaseUrl"] ?? "https://www.courtlistener.com/api/rest/v3/";
    ...
    public async Task<OpinionSearchResult> SearchOpinionsAsync(Dictionary<string, object> parameters)
    {
        ...
        var url = $"{_baseUrl}opinions/?{queryString}";
    ...
    {
        var queryString = BuildQueryString(parameters);
        var url = $"{_baseUrl}dockets/?{queryString}";
