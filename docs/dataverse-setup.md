...
> **Note:** Use the display name (e.g., `$DATAVERSE_ENV_DISPLAY_NAME`) in place of the variable.
...
New-AdminPowerAppEnvironment `
  -DisplayName $DATAVERSE_ENV_DISPLAY_NAME `
...
az ad app create \
   --display-name "$DATAVERSE_APP_DISPLAY_NAME" \
   --sign-in-audience AzureADMyOrg

# Get the App ID
DATAVERSE_APP_ID=$(az ad app list --display-name "$DATAVERSE_APP_DISPLAY_NAME" --query "[0].appId" -o tsv)
echo "Dataverse App ID: $DATAVERSE_APP_ID"

# Create client secret
DATAVERSE_APP_SECRET=$(az ad app credential reset \
   --id $DATAVERSE_APP_ID \
   --append \
   --display-name "DataverseSecret" \
   --query "password" \
   --output tsv)
echo "Dataverse Secret: $DATAVERSE_APP_SECRET"
...
5. Click **+ Add an app**
6. Search for your app registration (`$DATAVERSE_APP_DISPLAY_NAME`) and select it
...
4. Configure the table:
   - **Display name:** `$DATAVERSE_TABLE_DISPLAY_NAME`
   - **Plural name:** `$DATAVERSE_TABLE_PLURAL_NAME`
   - **Name:** `$DATAVERSE_TABLE_NAME`
   - **Primary column:**
     - **Display name:** `Name`
     - **Name:** `name`
5. Click **Save**
...
> **Note:** The prefix `cr3f3_` is auto-generated by Dataverse. Your prefix may be different.

## 7. Set Table Schema Variable

Once you've verified the table schema name, set it as a variable:

```bash
# Replace with your actual prefix (check Power Apps for the prefix)
DATAVERSE_TABLE_PREFIX="cr3f3"  # Your prefix may be different!

# Construct the full table name
DATAVERSE_TABLE_SCHEMA="${DATAVERSE_TABLE_PREFIX}_${DATAVERSE_TABLE_NAME}"
echo "Full Dataverse table schema: $DATAVERSE_TABLE_SCHEMA"
```
...
4. Find your table and click on it
5. Go to Settings > Advanced options
6. Look at the Name field - it will show something like cr3f3_courtlistenercache
7. The prefix is everything before the underscore (e.g., cr3f3)

Save this value - you'll use `$DATAVERSE_TABLE_SCHEMA` when configuring the MCP server.
...
3. **Custom Table:**
   - Table Name: `<${DATAVERSE_TABLE_PREFIX}>_<$DATAVERSE_TABLE_NAME>` (or your prefix)
